    ...
    async function handleCreateNewHabit() {
        try {
            if (!title.trim() || weekDays.length === 0) {
                return Alert.alert('Novo Hábito', 'Informe o nome do novo hábito e escolha a peoridicidade!')
            }
    ...
    return (
        <View className="flex-1 bg-background px-8 pt-16">
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
...

                {
                    avaiableWeekDays.map((weekDay, index) => (
                        <Checkbox
                            key={weekDay}
                            title={weekDay}
                            onPress={() => handleToggleWeekDay(index)} checked={weekDays.includes(index)}
                        />
                    ))
                }
                ...
                <TouchableOpacity
                    activeOpacity={0.7}
                    className="wfull h-14 flex-row items-center justify-center bg-green-600 rounded-md mt-6"
